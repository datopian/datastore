...
7. **Sign URLs** — V4 with `response-content-disposition: attachment; filename="<rid>.<ext>"` (one file) or `<rid>_NN.<ext>` (multi-file parquet, 1-indexed). Signing is offloaded to a thread (IAM round-trip under workload identity) and runs as the **ro** identity: the signer's SA email is public in the signed URL, so the account named there should be one that can only read.
...
| `generate_signed_url` | **RO GCS** | A V4 URL carries the signer's SA email in `X-Goog-Credential`, where it is public, and the grant it issues is bounded by what that identity can do. Signing a GET with the rw SA advertises the write identity and issues a grant from an account that could also overwrite the object — so `_signed_urls` re-binds each blob to the ro bucket first. Requires `iam.serviceAccountTokenCreator` **on the ro SA** (IAM `signBlob` under workload identity) plus `storage.objects.get`. |

Concrete perm sets:

- **RO SA** (`BIGQUERY_CREDENTIALS_RO`) — `bigquery.tables.get` + `storage.objects.{list,get}` + `iam.serviceAccountTokenCreator` (on itself, for signing).
- **RW SA** (`BIGQUERY_CREDENTIALS`) — `bigquery.jobs.create` + `bigquery.tables.export` + `bigquery.tables.getData` + `storage.objects.{create,get,list,delete}`. **`get` is required by `compose`** (it reads each source object); a role with only create/list/delete 403s on the compose step.
